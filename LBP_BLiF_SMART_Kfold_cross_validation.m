...
for i = 1:16
    input_code_NR{i,:}=input_NR(((i-1)*16+1):i*16,:);
    output_code{i,:}=output(((i-1)*16+1):i*16,:);
end
...
for fork=1:all_num
    ...
    %test data
    for m=1:size(test',1)
        input_test_NR(1+16*(m-1):(16*m),:)=input_code_NR{test(m)};
        output_test(1+16*(m-1):(16*m),:)=output_code{test(m)};
    end
    %trainging data
    for n=1:size(train',1)
        input_train_NR(1+16*(n-1):(16*n),:)=input_code_NR{train(n)};
        output_train(1+16*(n-1):(16*n),:)=output_code{train(n)};
    end       
   %% normalization
